...
const MoleSkull = ({data}: {data: any}) => {
    ...
    useEffect(() => {
        ...
        rotateAroundCenter(pathRef.current, data.rt)
    }, []);

    function handleSkullClick() {
        if (pathRef.current.clicked) return;
        pathRef.current.to({
            scaleX: 0.1,
            scaleY: 0.1,
            duration: 0.8,
        ...
        pathRef.current.clicked = true
        data.handleImageClick({path: pathRef.current, pointer: pathRef.current.getStage().getPointerPosition()})
    }

    const rotatePoint = ({ x, y }: any, rad: any) => {
...
// will work for shapes with top-left origin, like rectangle
    ...
    return (
        <Path ref={pathRef}
              ...
              shadowColor={'#060b10cf'}
              shadowBlur={40}
              shadowOffset={{ x: 50, y: 70 }}
